...
switch ( strtolower($op) ) {
    case "upload":
        if ( $_FILES['uploadfile']['name'] != "" ) {
            if ( file_exists( XOOPS_ROOT_PATH . "/" . $_POST['uploadpath'] . "/" . $_FILES['uploadfile']['name'] ) ) {
                redirect_header( 'upload.php', 2, _AM_WFB_LINK_IMAGEEXIST );
            } 
            $allowed_mimetypes = array( 'image/gif', 'image/jpeg', 'image/pjpeg', 'image/x-png', 'image/png' );
            wfl_uploading( $_FILES, $_POST['uploadpath'], $allowed_mimetypes, "upload.php", 1, 0 );
            redirect_header( "upload.php", 2 , _AM_WFB_LINK_IMAGEUPLOAD );
            exit();
        } else {
            redirect_header( 'upload.php', 2 , _AM_WFB_LINK_NOIMAGEEXIST );
            exit();
        } 
        break;
        ...
        if ( isset( $_POST['confirm'] ) && $_POST['confirm'] == 1 ) {
            ...
            if ( file_exists( $filetodelete ) ) {
                ...
                if ( @unlink( $filetodelete ) ) {
                    redirect_header( 'upload.php', 1, _AM_WFB_LINK_FILEDELETED );
                } else {
                    redirect_header( 'upload.php', 1, _AM_WFB_LINK_FILEERRORDELETE );
                } 
            } 
            exit();
        } else {
            if ( empty( $_POST['linkfile'] ) ) {
                redirect_header( 'upload.php', 1, _AM_WFB_LINK_NOFILEERROR );
                exit();
            } 
            xoops_cp_header();
            xoops_confirm( array( 'op' => 'delfile', 'uploadpath' => $_POST['uploadpath'], 'linkfile' => $_POST['linkfile'], 'confirm' => 1 ), 'upload.php', _AM_WFB_LINK_DELETEFILE . "<br /><br />" . $_POST['linkfile'], _AM_WFB_BDELETE );
        } 
        break;
    ...
    default:
        ...
        $namearray = array( 1 => _AM_WFB_LINK_CATIMAGE , 2 => _AM_WFB_LINK_SCREENSHOTS, 3 => _AM_WFB_LINK_MAINIMAGEDIR );
        $listarray = array( 1 => _AM_WFB_LINK_FCATIMAGE , 2 => _AM_WFB_LINK_FSCREENSHOTS, 3 => _AM_WFB_LINK_FMAINIMAGEDIR );

        wfl_adminmenu( _AM_WFB_MUPLOADS );
        wfl_serverstats();
        if ( $rootpath > 0 ) {
            echo "<div><b>" . _AM_WFB_LINK_FUPLOADPATH . "</b> " . XOOPS_ROOT_PATH . "/" . $dirarray[$rootpath] . "</div>\n";
            echo "<div><b>" . _AM_WFB_LINK_FUPLOADURL . "</b> " . XOOPS_URL . "/" . $dirarray[$rootpath] . "</div><br />\n";
        } 
        $pathlist = ( isset( $listarray[$rootpath] ) ) ? $namearray[$rootpath] : '';
        $namelist = ( isset( $listarray[$rootpath] ) ) ? $namearray[$rootpath] : '';

        $iform = new XoopsThemeForm( _AM_WFB_LINK_FUPLOADIMAGETO . $pathlist, "op", xoops_getenv( 'PHP_SELF' ) );
        $iform -> setExtra( 'enctype="multipart/form-data"' );
        ob_start();
        $iform -> addElement( new XoopsFormHidden( 'dir', $rootpath ) );
        wfl_getDirSelectOption( $namelist, $dirarray, $namearray );
        $iform -> addElement( new XoopsFormLabel( _AM_WFB_LINK_FOLDERSELECTION, ob_get_contents() ) );
        ob_end_clean();
        ...
        if ( $rootpath > 0 ) {
            ...
            $indeximage_tray = new XoopsFormElementTray( _AM_WFB_LINK_FSHOWSELECTEDIMAGE, '&nbsp;' );
            ...
            $iform -> addElement( new XoopsFormFile( _AM_WFB_LINK_FUPLOADIMAGE, 'uploadfile', 0 ) );
            ...
            $butt_dup = new XoopsFormButton( '', '', _AM_WFB_BUPLOAD, 'submit' );
            $butt_dup -> setExtra( 'onclick="this.form.elements.op.value=\'upload\'"' );
            $dup_tray -> addElement( $butt_dup );

            $butt_dupct = new XoopsFormButton( '', '', _AM_WFB_BDELETEIMAGE, 'submit' );
