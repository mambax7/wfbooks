...
switch ( strtolower( $op ) ) {
    case "updatenotice":
        ...
        if ( $ack && !$con ) {
            ...
            $sql = "UPDATE " . $xoopsDB -> prefix( 'wfbooks_broken' ) . " SET acknowledged=" . $acknowledged;
            if ( $acknowledged == 0 ) {
                $sql .= ", confirmed=0 ";
            } 
            $sql .= " WHERE lid=" . $lid;
            ...
            $update_mess = _AM_WFB_BROKEN_NOWACK;
        } 

        if ( $con ) {
            $confirmed = ( $con == 0 ) ? 1 : 0;
            $sql = "UPDATE " . $xoopsDB -> prefix( 'wfbooks_broken' ) . " SET confirmed=" . $confirmed;
            if ( $confirmed == 1 ) {
            ...
            $update_mess = _AM_WFB_BROKEN_NOWCON;
        } 
        redirect_header( "brokenlink.php?op=default", 1, $update_mess );
        break;

    case "delbrokenlinks":
        $xoopsDB -> queryF( "DELETE FROM " . $xoopsDB -> prefix( 'wfbooks_broken' ) . " WHERE lid=" . $lid );
        $xoopsDB -> queryF( "DELETE FROM " . $xoopsDB -> prefix( 'wfbooks_links' ) . " WHERE lid=" . $lid );
        redirect_header( "brokenlink.php?op=default", 1, _AM_WFB_BROKENFILEDELETED );
        exit();
        break;

    case "ignorebrokenlinks":
        $xoopsDB -> queryF( "DELETE FROM " . $xoopsDB -> prefix( 'wfbooks_broken' ) . " WHERE lid=" . $lid );
        redirect_header( "brokenlink.php?op=default", 1, _AM_WFB_BROKEN_FILEIGNORED );
        break;

    default:
        $result = $xoopsDB -> query( "SELECT * FROM " . $xoopsDB -> prefix( 'wfbooks_broken' ) . " ORDER BY reportid" );
        $totalbrokenlinks = $xoopsDB -> getRowsNum( $result );

        xoops_cp_header();
        wfl_adminmenu( _AM_WFB_BROKEN_FILE );
        echo "
		<fieldset>
		 <legend style='font-weight: bold; color: #0A3760;'>" . _AM_WFB_BROKEN_REPORTINFO . "</legend>\n
		  <div style='padding: 8px;'>" . _AM_WFB_BROKEN_REPORTSNO . "&nbsp;<b>$totalbrokenlinks</b><div>\n
		  <div style='padding: 8px;'>\n
		   <ul>
		    <li>" . $imagearray['ignore'] . " " . _AM_WFB_BROKEN_IGNOREDESC . "</li>\n
		    <li>" . $imagearray['editimg'] . " " . _AM_WFB_BROKEN_EDITDESC . "</li>
		    <li>" . $imagearray['deleteimg'] . " " . _AM_WFB_BROKEN_DELETEDESC . "</li>\n
		   </ul>
		  </div>\n
		 </fieldset>
		<br />\n
		...
		<th width='3%' style='text-align: center;'>" . _AM_WFB_BROKEN_ID . "</th>\n
		<th width='35%' style='text-align: left;'>" . _AM_WFB_BROKEN_TITLE . "</th>\n
		<th>" . _AM_WFB_BROKEN_REPORTER . "</th>\n
		<th>" . _AM_WFB_BROKEN_FILESUBMITTER . "</th>\n
		<th>" . _AM_WFB_BROKEN_DATESUBMITTED . "</th>\n
		<th>" . _AM_WFB_BROKEN_ACKNOWLEDGED . "</th>\n
		<th>" . _AM_WFB_BROKEN_DCONFIRMED . "</th>\n		
		<th style='text-align: center;'>" . _AM_WFB_BROKEN_ACTION . "</th>\n
		</tr>\n";

        if ( $totalbrokenlinks == 0 ) {
            echo "<tr style='text-align: center;'><td style='text-align: center;' class='head' colspan='8'>" . _AM_WFB_BROKEN_NOFILEMATCH . "</td></tr>";
        } else {
            while ( list( $reportid, $lid, $sender, $ip, $date, $confirmed, $acknowledged ) = $xoopsDB -> fetchRow( $result ) ) {
                $result2 = $xoopsDB -> query( "SELECT cid, title, url, submitter FROM " . $xoopsDB -> prefix( 'wfbooks_links' ) . " WHERE lid=$lid" );
                ...
                echo "<td class='even'><a href='brokenlink.php?op=updateNotice&amp;lid=$lid&amp;ack=$acknowledged'>" . $ack_image . " </a></td>\n";
