...
$xoopsOption['template_main'] = 'wfbooks_topten.html';
...
$lang_array 	= array( _MD_WFB_HITS, _MD_WFB_RATING );
$rankings 	= array();
...
$arr = array();
$result = $xoopsDB -> query( "SELECT cid, title, pid FROM " . $xoopsDB -> prefix( 'wfbooks_cat' ) . " WHERE pid=0" );

$e = 0;
while ( list( $cid, $ctitle ) = $xoopsDB -> fetchRow( $result ) ) {
    if ( true == checkgroups( $cid ) ) {
        $query = "SELECT lid, cid, title, hits, rating, votes FROM " . $xoopsDB -> prefix( 'wfbooks_links' ) . " WHERE published > 0 AND published <= " . time() . " AND (expired = 0 OR expired > " . time() . ") AND offline = 0 AND (cid=" . intval($cid);
